...
const App: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    if (state.view === 'settings') return; // Don't fetch SEO data on settings page

    setLoading(true);
    try {
      if (state.view === 'overview') {
        const result = await DataService.getOverview(state.startDate, state.endDate);
        setData(result);
      } else if (state.view === 'pageDetail' && state.selectedUrl) {
        const result = await DataService.getUrlDetails(state.selectedUrl, state.startDate, state.endDate);
        setDetailData(result);
      } else if (state.view === 'queryDetail' && state.selectedQuery) {
        const result = await DataService.getQueryDetails(state.selectedQuery, state.startDate, state.endDate);
        setDetailData(result);
      }
    } catch (err) {
      console.error(err);
  ...
  }, [state.view, state.startDate, state.endDate, state.selectedUrl, state.selectedQuery]);
  ...
  const handleConfigUpdate = (newConfig: BQConfig) => {
    ...
    setState(prev => ({ ...prev, config: newConfig, view: 'overview' }));
    // In a real app, this would trigger a re-connection or validate credentials
  };
  ...
  return (
    <div className="min-h-screen pb-12 bg-slate-50/50">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            {/* Date Picker Trigger (Only visible on analysis pages) */}
            {state.view !== 'settings' && (
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {state.view === 'settings' ? (
          <SettingsView config={state.config} onSave={handleConfigUpdate} />
        ) : loading ? (
          <div className="flex flex-col items-center justify-center py-32 space-y-6">
            ...
            <div className="text-center">
              ...
              <p className="text-slate-400 text-sm font-medium">SEO metrikleri anlık olarak hesaplanıyor...</p>
            </div>
          </div>
        ) : (
          <>
      ...
      <footer className="max-w-7xl mx-auto px-6 py-12 text-center border-t border-slate-100 mt-20">
        <div className="inline-flex items-center gap-4 px-6 py-3 bg-white rounded-2xl border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${state.config.projectId ? 'bg-emerald-500 animate-pulse' : 'bg-amber-400'}`}></span>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              {state.config.projectId ? 'BigQuery Engine Connected' : 'Configuration Required'}
            </span>
          </div>
